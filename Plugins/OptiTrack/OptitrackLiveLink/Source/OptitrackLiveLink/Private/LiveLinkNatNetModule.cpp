...
#define LOCTEXT_NAMESPACE "OptitrackLiveLinkNatNet"
...
DEFINE_LOG_CATEGORY( LogOptitrackLiveLink );


class FLiveLinkNatNetModule : public IOptitrackLiveLinkPlugin
{
public:
	...
	// IOptitrackLiveLinkPlugin implementation
	virtual NatNetClient* CreateClient() override;
	virtual void DestroyClient( NatNetClient* pClient ) override;
...
static void NATNET_CALLCONV NatNetLog( Verbosity verbosity, const char* message )
{
	...
	switch ( verbosity )
	{
		case Verbosity_Error:
			UE_LOG( LogOptitrackLiveLink, Error, TEXT( "[NatNet - %s]: %s" ), levelStr, *tbuf );
			break;
		case Verbosity_Warning:
			UE_LOG( LogOptitrackLiveLink, Warning, TEXT( "[NatNet - %s]: %s" ), levelStr, *tbuf );
			break;
		default:
			UE_LOG( LogOptitrackLiveLink, Log, TEXT( "[NatNet - %s]: %s" ), levelStr, *tbuf );
			break;
	}
}
...
void FLiveLinkNatNetModule::StartupModule()
{
#if NATNET_LOAD_DLL
	// Note: These paths correspond to the RuntimeDependency specified in the .Build.cs script.
	const FString PluginBaseDir = IPluginManager::Get().FindPlugin( "OptitrackLiveLink" )->GetBaseDir();
...
#	error Path to NatNet shared library not specified for this platform!
	...
	UE_LOG( LogOptitrackLiveLink, Log, TEXT( "FLiveLinkNatNetModule: NatNetLib loaded, version %d.%d.%d.%d" ), clientVer[0], clientVer[1], clientVer[2], clientVer[3] );
#endif // !UE_BUILD_SHIPPING
}
...
void FLiveLinkNatNetModule::ShutdownModule()
{
#if NATNET_LOAD_DLL
	// Unload the DLL.
	if ( NatNetLibHandle != nullptr )
...
NatNetClient* FLiveLinkNatNetModule::CreateClient()
{
...
void FLiveLinkNatNetModule::DestroyClient( NatNetClient* pClient )
{
	delete pClient;
}
...
IMPLEMENT_MODULE( FLiveLinkNatNetModule, OptitrackLiveLink )
